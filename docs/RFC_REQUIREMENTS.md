...
| `RFC2131-4.3.5-MUST-NOT-01` | [4.3.5](https://www.rfc-editor.org/rfc/rfc2131.html#section-4.3.5) | MUST NOT | **partial** | Do not include a lease expiration time in the DHCPACK for DHCPINFORM. | [Client receives configuration options via DHCPINFORM](../features/dhcp_inform.feature)<br>The scenario proves no address assignment but does not directly assert absence of option 51. |
...
| ID | Section | Level | Status | Requirement | Evidence / rationale |
|---|---:|---|---|---|---|
| `RFC2132-3.3-MUST-01` | [3.3](https://www.rfc-editor.org/rfc/rfc2132.html#section-3.3) | MUST | **partial** | Place the Subnet Mask option before the Router option when both are returned. | [DHCPACK includes required network configuration options](../features/dhcp_options.feature)<br>Both options are required by the scenario but their wire ordering is not asserted. |
| `RFC2132-3.8-MUST-01` | [3.8](https://www.rfc-editor.org/rfc/rfc2132.html#section-3.8) | MUST | **partial** | Encode the Domain Name Server option length as a multiple of four octets. | [DHCPACK includes required network configuration options](../features/dhcp_options.feature)<br>Successful address decoding gives partial format coverage but multiple DNS addresses are not exercised. |
| `RFC2132-9.2-MUST-01` | [9.2](https://www.rfc-editor.org/rfc/rfc2132.html#section-9.2) | MUST | **partial** | Encode the IP Address Lease Time as a four-octet unsigned value. | [DHCPACK includes correct T1 and T2 lease timer values](../features/dhcp_options.feature)<br>The lease and timer values are decoded but the raw option length is not separately asserted. |
